									...
									<div class="text-xs-left mt-4">
										...
										<!--     <v-layout row wrap mt-2 pa-0 mb-3>
											 ...
											 <v-layout row wrap mt-2 pa-0 mb-3>
												...
												<v-spacer></v-spacer>
												<h4 class="font-weight-bold"> $ {{ currency(inquiry.desired_price) }} </h4>
											</v-layout>
						...
						<v-card class="pa-4" elevation=3>
							...
							<v-flex xs12>
							 ...
							 <v-flex xs12>
								<v-layout row wrap mt-2 pa-0 mb-3>
									...
									<v-spacer></v-spacer>
									<h2 class="font-weight-bold">$ {{ currency(this.bid.price) }}</h2>
								</v-layout>
							</v-flex>
							...
							<v-flex xs12>
								<v-layout row wrap mt-3>
									...
									<v-spacer></v-spacer>
									<h1 class="font-weight-bold green--text">$ {{ currency(bid.total_price) }}</h1>
								</v-layout>
							</v-flex>
		...
		submit() {
			...
			if(!this.$v.form.$error) {
				...
				.then((respose) => {
					...
					// set the Inquiry store state inquiry.awarded to 1
					var inqTmp = this.inquiry;
					inqTmp.awarded = 1;
					this.stateInquiry = inqTmp;

					inqEvntBs.emitBidAwarded();
				})
				.catch((e) => {
					this.formLoading = false;
				})
